...
import { FaCheckCircle, FaHourglassHalf, FaTimesCircle, FaQuestionCircle } from "react-icons/fa";
  ...
  return (
    <div className="flex flex-wrap gap-4">
      {icons.map(({ name, icon: Icon }) => (
        <div key={name} className={`p-2 cursor-pointer ${selectedIcon === name ? "border-2 border-blue-500" : ""}`} onClick={() => onSelectIcon(name)}>
