...
const ClientSide = ({ activities: initialActivities }: ClientSideProps) => {
  ...
  //     setActivities((prevActivities) => prevActivities.map((activity) => (activity._id === updatedActivity._id ? updatedActivity : activity)));
  ...
  return (
    <>
      <div className="space-y-4">
        {activities.length === 0 ? (
          <div className="text-center p-4 border border-dashed border-gray-300 rounded-lg">
            <p className="text-xl font-semibold text-gray-700">No activities yet. Start by adding a new activity!</p>
          </div>
        ) : (
          activities.map((activity) => <ActivityCard key={activity._id} activity={activity} onClick={() => handleActivityClick(activity)} />)
        )}
      </div>
      <Sidebar
      ...
      <div className="sticky bottom-0 bg-white p-4 border-t border-gray-300 max-w-4xl mx-auto w-full mt-16">
          <button onClick={handleAddTask} className="flex items-center bg-orange-100 text-opacity-80 text-black py-4 px-5 rounded-2xl w-full justify-start text-xl font-semibold border border-gray-200">
